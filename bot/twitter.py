...
from bot.db.latest_quote import get_latest_quote_in_past_7_days
from bot.db.latest_reply import get_latest_reply_in_past_7_days
from bot.db.latest_retweet import get_latest_retweet_in_past_7_days
from bot.db.latest_tweet import get_days_ago_tweet_ids, get_latest_tweet_in_past_7_days
...
def extract_and_save_tweets(
    ...
    *Note:* Either one of the `user_id` or the `username` should be given.
    ...
    latest_mention_id = get_latest_mention_in_past_7_days(user_id=str(user_id))
    ...
    latest_tweet_id = get_latest_tweet_in_past_7_days(user_id=str(user_id))
    ...
    for tweet in user_tweets:
        ...
        if referenced_tweets and "retweeted" in referenced_tweets:
            ...
            latest_reply_id = get_latest_reply_in_past_7_days(tweet_id=tweet.id)
            ...
            latest_quote_id = get_latest_quote_in_past_7_days(tweet_id=tweet.id)
            quotes_of_reply = get_quotes_of_tweet(
                tweet_id=tweet.id, since_id=latest_quote_id
            )
            save_tweets_in_neo4j(quotes_of_reply)

            latest_retweet_id = get_latest_retweet_in_past_7_days(tweet_id=tweet.id)
