...
def get_twitter_user(id=None, username=None) -> tweepy.User:
    ...
    logging.info(f"Start fetching `ّUser` information of a User with ID {id} or USERNAME {username}, It might take long (because of twitter api limits)")
...
def get_twitter_users(ids=None, usernames=None) -> list[tweepy.User]:
    ...
    logging.info(f"Start fetching `ّUsers` information of Users with IDs {ids} or USERNAMEs {usernames}, It might take long (because of twitter api limits)")
    ...
    logging.info(f"information of `Users` with ID {ids} or USERNAME {usernames} were fetched")
    return user_data
