...
def test_get_latest_quote_userid_input_output_none():
    ...
    id = get_latest_quote_since(user_id="12345")
...
def test_get_latest_quote_tweetId_input_output_none():
    ...
    id = get_latest_quote_since(tweet_id="12345")
...
def test_get_latest_quote_empty_inputs_output_none():
    ...
    try:
        _ = get_latest_quote_since(user_id=None, tweet_id=None)
...
def test_get_latest_quote_userid_input():
    ...
    created_time2 = int((datetime.now() - timedelta(days=4)).timestamp() * 1000)
    neo4j_ops.gds.run_cypher(
        f"""
        ...
        SET r5.createdAt = {created_time2}
        """
    )

    id = get_latest_quote_since(user_id="989898")
...
def test_get_latest_quote_tweetid_input():
    ...
    id = get_latest_quote_since(tweet_id="1111")
